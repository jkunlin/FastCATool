...
TestSetFile::TestSetFile(const string &filename)
{
	...
	while (getline(infile, line))
	{
		if (line.size() == 0)
			continue;
		regex ws_res(",");
		vector<string> v(sregex_token_iterator(line.begin(), line.end(), ws_res, -1), sregex_token_iterator());
		vector<int> st;
		for (auto &&s : v)
		{
			if (s.compare("*") == 0)
...
unsigned int TestSetFile::getTestSetCount()
{
...
bool TestSetFile::IsThisRow(unsigned inSetIndex, vector<unsigned> &rowInResult)
{
	for (unsigned i = 0; i < var_count; i++)
	{
		if (testSet[inSetIndex][i] > 0 && rowInResult[i] > 0 && testSet[inSetIndex][i] != rowInResult[i])
			return false;
	}
	return true;
}
